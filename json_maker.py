...
for key in ratings:
    value = ratings[key]
    family = value[0]
    ratings_sum = sum(value)
    family_friendly = family/ratings_sum
    if family > 0.3:
        final_ratings[key] = "Yes"
    else:
        final_ratings[key] = "No"
...
museum_info = {}
for m in museums:
    if not(m in final_ratings):
        final_ratings[m] = None
    if not(m in tags):
        tags[m] = None
    ...
    museum_info[m] = {'description': description[m], 'ratings': final_ratings[m], 'tags': tags[m], 'tokenized tags': tok_tags[m],
                      'review titles': review_titles[m], 'review content': review_content[m], 'tokenized content': tok_review[m], 'location': location[m], "free": free[m]}
